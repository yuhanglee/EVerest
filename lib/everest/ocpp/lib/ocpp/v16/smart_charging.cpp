...
bool validate_schedule(const ChargingSchedule& schedule, const int charging_schedule_max_periods,
    ...
    if (schedule.minChargingRate.has_value() && !std::isfinite(schedule.minChargingRate.value())) {
        EVLOG_warning << "INVALID SCHEDULE - Non-finite minChargingRate: " << schedule.minChargingRate.value();
        return false;
    }
