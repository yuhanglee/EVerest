...
#define VALUE(a, b) {valid_keys::a, &ocpp::v2::ControllerComponentVariables::b},

static const std::pair<valid_keys, const ocpp::v2::ComponentVariable*> max_limit_entries[] = {MAPPING_MAX_LIMIT(VALUE)};
...
void for_each_max_limit_key(const std::function<void(valid_keys)>& fn) {
    for (const auto& [key, cv] : max_limit_entries) {
        fn(key);
    }
}
