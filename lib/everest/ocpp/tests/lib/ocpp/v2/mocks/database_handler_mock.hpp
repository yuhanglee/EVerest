...
public:
    ...
    MOCK_METHOD(void, insert_or_update_der_control,
                (const std::string& control_id, bool is_default, const std::string& control_type,
                 bool is_superseded, int32_t priority, const std::optional<std::string>& start_time,
                 const std::optional<float>& duration, const std::string& control_json),
                (override));
